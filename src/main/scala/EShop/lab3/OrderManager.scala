  ...
  def open(
    ...
    msg match {
      ...
      case RemoveItem(id, sender) =>
        ...
        open(cartActor)
      case Buy(sender) =>
        val orderManagerToCartRef: ActorRef[TypedCartActor.Event] =
          ctx.messageAdapter {
  ...
  def inCheckout(
    ...
    msg match {
      case SelectDeliveryAndPaymentMethod(delivery, payment, sender) =>
        val orderManagerToCheckoutRef: ActorRef[TypedCheckout.Event] =
          ctx.messageAdapter {
            case TypedCheckout.PaymentStarted(payment) =>
              ConfirmPaymentStarted(payment)
          }
        val orderManagerToPaymentRef: ActorRef[Payment.Event] =
          ctx.messageAdapter {
